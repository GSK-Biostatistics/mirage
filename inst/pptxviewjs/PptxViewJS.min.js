...
(function (global) {
  ...
  function getBodyPrAnchorFromElement(txBodyEl) {
    if (!txBodyEl || typeof txBodyEl.querySelector !== "function") return null;

    var bodyPr = txBodyEl.querySelector("bodyPr, a\\:bodyPr");
    if (!bodyPr) return null;

    return bodyPr.getAttribute("anchor");
  }

  function ensureTextBodyAlignment(textBody, txBodyEl) {
    if (!textBody) return textBody;

    var bodyProps = textBody.bodyProperties || textBody.bodyPr || {};
    var anchor = normalizeOoxmlVerticalAnchor(
      bodyProps.anchor != null
        ? bodyProps.anchor
        : bodyProps.verticalAlign != null
          ? bodyProps.verticalAlign
          : getBodyPrAnchorFromElement(txBodyEl)
    );

    bodyProps.anchor = anchor;
    bodyProps.verticalAlign = anchor;

    if (bodyProps.wrap == null) bodyProps.wrap = true;
    if (bodyProps.leftMargin == null) bodyProps.leftMargin = 45720;
    if (bodyProps.rightMargin == null) bodyProps.rightMargin = 45720;
    if (bodyProps.topMargin == null) bodyProps.topMargin = 22860;
    if (bodyProps.bottomMargin == null) bodyProps.bottomMargin = 22860;

    textBody.bodyProperties = bodyProps;

    if (textBody.bodyPr && textBody.bodyPr !== bodyProps) {
      textBody.bodyPr.anchor = anchor;
      textBody.bodyPr.verticalAlign = anchor;
    }

    return textBody;
  }

  function patchPPTXSlideRenderer() {
    var Ctor = global.PPTXSlideRenderer;
    if (!Ctor || !Ctor.prototype || Ctor.prototype.__ooxmlValignPatched) return false;

    var proto = Ctor.prototype;

    if (typeof proto.parseBodyProperties === "function") {
      var originalParseBodyProperties = proto.parseBodyProperties;
      proto.parseBodyProperties = function (txBodyEl) {
        var props = originalParseBodyProperties.call(this, txBodyEl) || {};
        var anchor = normalizeOoxmlVerticalAnchor(
          props.anchor != null
            ? props.anchor
        ...
        if (props.wrap == null) props.wrap = true;
        if (props.leftMargin == null) props.leftMargin = 45720;
        if (props.rightMargin == null) props.rightMargin = 45720;
        if (props.topMargin == null) props.topMargin = 22860;
        if (props.bottomMargin == null) props.bottomMargin = 22860;

        return props;
      };
    }

    if (typeof proto.processTextBody === "function") {
      var originalProcessTextBody = proto.processTextBody;
      proto.processTextBody = function (txBodyEl) {
        var textBody = originalProcessTextBody.call(this, txBodyEl);
        return ensureTextBodyAlignment(textBody, txBodyEl);
      };
    }

    Ctor.prototype.__ooxmlValignPatched = true;
    return true;
  }

  function patchCDrawingDocument() {
    var Ctor = global.CDrawingDocument;
    if (!Ctor || !Ctor.prototype || Ctor.prototype.__ooxmlValignPatched) return false;

    var proto = Ctor.prototype;

    if (typeof proto.renderTextBodyStandard === "function") {
      var originalRenderTextBodyStandard = proto.renderTextBodyStandard;
      proto.renderTextBodyStandard = function (textBody, bounds) {
        ensureTextBodyAlignment(textBody, null);
        return originalRenderTextBodyStandard.call(this, textBody, bounds);
      };
    }
  ...
  function patchTableProcessor() {
    var Ctor = global.TableProcessor;
    if (!Ctor || !Ctor.prototype || Ctor.prototype.__ooxmlValignPatched) return false;

    var proto = Ctor.prototype;

    if (typeof proto.parseTextBodyFromElement === "function") {
      var originalParseTextBodyFromElement = proto.parseTextBodyFromElement;
      proto.parseTextBodyFromElement = function (el) {
        var textBody = originalParseTextBodyFromElement.call(this, el);
        return ensureTextBodyAlignment(textBody, el);
      };
    }

    if (typeof proto.renderCellTextContent === "function") {
      var originalRenderCellTextContent = proto.renderCellTextContent;
      proto.renderCellTextContent = function (graphics, textBody, x, y, w, h, cell) {
        ensureTextBodyAlignment(textBody, null);

        if (cell) {
          cell.verticalAlignment = normalizeOoxmlVerticalAnchor(
            cell.verticalAlignment != null
              ? cell.verticalAlignment
              : textBody &&
                  textBody.bodyProperties &&
                  textBody.bodyProperties.anchor != null
                ? textBody.bodyProperties.anchor
                : textBody &&
                    textBody.bodyProperties &&
                    textBody.bodyProperties.verticalAlign != null
                  ? textBody.bodyProperties.verticalAlign
                  : "t"
          );
        }

        return originalRenderCellTextContent.call(this, graphics, textBody, x, y, w, h, cell);
      };
    }
  ...
  function installPatches() {
    var patched = false;
    patched = patchPPTXSlideRenderer() || patched;
    patched = patchCDrawingDocument() || patched;
    patched = patchTableProcessor() || patched;
    return patched;
  }
  ...
  function tryInstall() {
    if (installPatches()) {
      global.__OOXML_VERTICAL_ALIGN_PATCH__ = {
        installed: true,
        normalizeOoxmlVerticalAnchor: normalizeOoxmlVerticalAnchor
      };
      return;
    }
...
/* ooxml-empty-line-bullet-fix-clean.js
   IMPORTANT:
   - Remove the previous empty-line bullet patch
   - Reload the page
   - Then load this patch after your pptx viewer bundle

   This patch preserves the original bullet indent/layout behavior
   and only suppresses bullets for empty paragraphs.
*/

(function (global) {
  "use strict";

  function runHasVisibleText(run) {
    if (!run) return false;
    if (run.fieldType) return true;
  ...
  function paragraphHasVisibleText(paragraph) {
    if (!paragraph || !Array.isArray(paragraph.runs)) return false;

    for (var i = 0; i < paragraph.runs.length; i++) {
      if (runHasVisibleText(paragraph.runs[i])) return true;
    }
    return false;
  }
  ...
  function installPatch() {
    ...
    if (Ctor.prototype.__emptyLineBulletFixCleanInstalled) return true;

    var proto = Ctor.prototype;

    if (typeof proto.parseParagraphProperties !== "function") return false;

    var originalParseParagraphProperties = proto.parseParagraphProperties;

    proto.parseParagraphProperties = function (paragraph, shape) {
      var props = originalParseParagraphProperties.call(this, paragraph, shape) || {};

      try {
        // If the paragraph has no visible text, suppress bullets entirely.
        // This preserves blank-line spacing but prevents:
        // - bullet rendering on empty lines
        // - numbering advancing on empty bullet paragraphs
        if (!paragraphHasVisibleText(paragraph)) {
          if (props.bullet && props.bullet.type && props.bullet.type !== "none") {
            props = Object.assign({}, props);
            props._originalBullet = props.bullet;
    ...
    Ctor.prototype.__emptyLineBulletFixCleanInstalled = true;
    return true;
  }
  ...
  function tryInstall() {
    if (installPatch()) {
      global.__OOXML_EMPTY_LINE_BULLET_FIX_CLEAN__ = { installed: true };
      return;
    }
...
/* ooxml-field-and-line-spacing-fix.js
   Replace the previous field patch with this file.
   Load AFTER your pptx viewer bundle and BEFORE rendering slides.
*/

(function (global) {
  "use strict";

  function isRgbObject(v) {
    return !!v &&
      typeof v === "object" &&
  ...
  function cloneRgb(c) {
    if (!isRgbObject(c)) return null;
    return {
      r: Number(c.r) || 0,
      g: Number(c.g) || 0,
      b: Number(c.b) || 0,
      a: typeof c.a !== "undefined" ? Number(c.a) : 255
    };
  }
  ...
  function normalizeColor(processor, c) {
    if (!c) return null;
    if (isRgbObject(c)) return cloneRgb(c);

    if (typeof c === "string") {
      var rgb = hexToRgb(c);
      if (rgb) return rgb;
      try {
        if (processor && typeof processor.resolveThemeColor === "function") {
          var resolved = processor.resolveThemeColor(c);
          if (isRgbObject(resolved)) return cloneRgb(resolved);
        }
      } catch (_) {}
      return null;
    }

    if (c.type === "srgb" && c.value) {
      return hexToRgb("#" + c.value);
    }

    if ((c.type === "scheme" || c.scheme) && (c.value || c.scheme)) {
      var scheme = c.value || c.scheme;
      try {
        if (processor && typeof processor.resolveThemeColor === "function") {
          var themeResolved = processor.resolveThemeColor(scheme);
          if (isRgbObject(themeResolved)) return cloneRgb(themeResolved);
        }
      } catch (_) {}
      ...
      try {
        var themeColors =
          ...
          processor.presentation.theme &&
          processor.presentation.theme.colors
            ? processor.presentation.theme.colors
            : null;

        if (themeColors && themeColors[scheme]) {
          if (typeof processor.parseColorFromHex === "function") {
            var parsed = processor.parseColorFromHex(themeColors[scheme]);
            if (isRgbObject(parsed)) return cloneRgb(parsed);
          }
          return hexToRgb(themeColors[scheme]);
        }
      } catch (_) {}
    }
  ...
  function mergeMissingRunProps(target, source, processor) {
    if (!source) return target;
    var out = target || {};

    if (!out.fontFamily && source.fontFamily) {
      out.fontFamily = source.fontFamily;
    }

    if ((out.fontSize == null || out.fontSize === 18 || out.fontSize === 12) &&
        source.fontSize != null) {
      out.fontSize = source.fontSize;
    }

    if (typeof out.bold === "undefined" && typeof source.bold !== "undefined") {
      out.bold = source.bold;
    }

    if (typeof out.italic === "undefined" && typeof source.italic !== "undefined") {
      out.italic = source.italic;
    }

    if ((!out.color || isBlackish(out.color)) && source.color) {
      var resolved = normalizeColor(processor, source.color);
  ...
  function getCanvasContext(doc) {
    if (!doc) return null;
    if (doc.graphics && doc.graphics.context) return doc.graphics.context;
    if (doc.graphics && doc.graphics._context) return doc.graphics._context;
    if (doc.graphics && doc.graphics.m_oContext) return doc.graphics.m_oContext;
    return null;
  }

  function getTextScaleFactorSafe(doc) {
    try {
      if (doc && typeof doc.getTextScaleFactor === "function") {
        var s = doc.getTextScaleFactor();
  ...
  function setCanvasFont(ctx, runProps, scaleFactor) {
    var fontSize = (runProps && runProps.fontSize != null ? runProps.fontSize : 12) * scaleFactor;
    var fontFamily = (runProps && runProps.fontFamily) ? runProps.fontFamily : "Arial";
    var bold = runProps && runProps.bold ? "bold" : "normal";
    var italic = runProps && runProps.italic ? "italic" : "normal";
  ...
  function measureRunLineBox(doc, runProps) {
    var ctx = getCanvasContext(doc);
    var scaleFactor = getTextScaleFactorSafe(doc);

    if (!ctx) {
      var fallbackFont = (runProps && runProps.fontSize != null ? runProps.fontSize : 12) * scaleFactor;
      return Math.max(fallbackFont * 1.2, fallbackFont + 2);
    }

    ctx.save();
    try {
      var fontPx = setCanvasFont(ctx, runProps || {}, scaleFactor);
      var m = ctx.measureText("MgÅyjQ");
      var ascent = typeof m.actualBoundingBoxAscent !== "undefined" ? m.actualBoundingBoxAscent : fontPx * 0.8;
      var descent = typeof m.actualBoundingBoxDescent !== "undefined" ? m.actualBoundingBoxDescent : fontPx * 0.25;
      var lineBox = ascent + descent;

      if (!isFinite(lineBox) || lineBox <= 0) {
        lineBox = fontPx * 1.2;
      }

      return Math.max(lineBox, fontPx * 1.05);
    } catch (_) {
      var fallback = (runProps && runProps.fontSize != null ? runProps.fontSize : 12) * scaleFactor;
      return Math.max(fallback * 1.2, fallback + 2);
    } finally {
      ctx.restore();
    }
  }

  function computeBaseLineHeight(doc, paraProps, wrappedLines) {
    var maxBox = 0;

    if (wrappedLines && wrappedLines.length) {
      for (var i = 0; i < wrappedLines.length; i++) {
        var line = wrappedLines[i];
        if (!line || !line.runs || !line.runs.length) continue;

        for (var j = 0; j < line.runs.length; j++) {
          var run = line.runs[j];
          var runProps = run && run.runProps ? run.runProps : paraProps || {};
          var box = measureRunLineBox(doc, runProps);
    ...
    if (maxBox <= 0) {
      maxBox = measureRunLineBox(doc, paraProps || {});
    }
  ...
  function installPatch() {
    var Ctor = global.CDrawingDocument;
    if (!Ctor || !Ctor.prototype) return false;
    if (Ctor.prototype.__fieldAndLineSpacingFixInstalled) return true;

    var proto = Ctor.prototype;

    if (typeof proto.parseRunProperties === "function") {
      var originalParseRunProperties = proto.parseRunProperties;

      proto.parseRunProperties = function (run, paraProps, shape) {
        var props = originalParseRunProperties.call(this, run, paraProps, shape) || {};

        try {
          var currentShape = shape;
          if (!currentShape && typeof this.getCurrentShape === "function") {
            currentShape = this.getCurrentShape();
          }

          var processor =
            this.processor ||
            (global.currentProcessor && global.currentProcessor.processor) ||
            null;

          var level = paraProps && typeof paraProps.level === "number" ? paraProps.level : 0;

          // Apply inheritance to field runs first, but also helps any run missing styling.
          if (typeof this.resolveInheritedTextProperties === "function" && currentShape) {
            try {
              var inherited = this.resolveInheritedTextProperties(currentShape, "run", level) || {};
              props = mergeMissingRunProps(props, inherited, processor);
            } catch (_) {}
          }

          // Paragraph defaults as fallback
          props = mergeMissingRunProps(props, paraProps || {}, processor);

          // If this is a field, be more aggressive about replacing generic fallback values
          if (run && run.fieldType) {
            if (paraProps && paraProps.fontSize != null &&
                (props.fontSize == null || props.fontSize === 18 || props.fontSize === 12)) {
              props.fontSize = paraProps.fontSize;
            }

            if (paraProps && paraProps.fontFamily && (!props.fontFamily || props.fontFamily === "Arial")) {
              props.fontFamily = paraProps.fontFamily;
            }

            if (paraProps && paraProps.color && (!props.color || isBlackish(props.color))) {
              var fieldColor = normalizeColor(processor, paraProps.color);
              if (fieldColor) props.color = fieldColor;
            }
          }
        } catch (_) {}
    ...
    if (typeof proto.calculateStandardLineHeight === "function") {
      var originalCalculateStandardLineHeight = proto.calculateStandardLineHeight;

      proto.calculateStandardLineHeight = function (paraProps, wrappedLines) {
        try {
          // Absolute line spacing in points
          if (paraProps && paraProps.lineHeightPoints) {
            var pxPerPt = 96 / 72;
            var scale = this.coordinateSystem && this.coordinateSystem.scale ? this.coordinateSystem.scale : 1;
            return paraProps.lineHeightPoints * pxPerPt * scale;
          }

          var base = computeBaseLineHeight(this, paraProps || {}, wrappedLines);

          // OOXML percentage line spacing
          var pct = paraProps && typeof paraProps.lineHeight === "number"
            ? paraProps.lineHeight
            : 100;
          ...
          // Prevent unrealistically tight overlap
          return Math.max(base, computed);
        } catch (_) {
          return originalCalculateStandardLineHeight.call(this, paraProps, wrappedLines);
        }
      };
    }

    Ctor.prototype.__fieldAndLineSpacingFixInstalled = true;
    return true;
  }
  ...
  function tryInstall() {
    if (installPatch()) {
      global.__OOXML_FIELD_AND_LINE_SPACING_FIX__ = { installed: true };
      return;
    }
...
/* ooxml-hide-fields.js
   Load AFTER your pptx viewer bundle and BEFORE loading/rendering the PPTX.

   Effect:
   - Hides auto-generated field text such as date/time and slide/page numbers
   - Works by blanking OOXML <a:fld> runs during parsing
*/
...
(function (global) {
  ...
  function isFieldRun(run) {
    return !!(run && run.fieldType);
  }

  function stripFieldRunsFromTextBody(textBody) {
    if (!textBody || !Array.isArray(textBody.paragraphs)) return textBody;

    for (var p = 0; p < textBody.paragraphs.length; p++) {
      var para = textBody.paragraphs[p];
      if (!para || !Array.isArray(para.runs)) continue;

      for (var r = 0; r < para.runs.length; r++) {
        var run = para.runs[r];
        if (!isFieldRun(run)) continue;

        run.text = "";
        run.hidden = true;
        run.suppressRender = true;

        if (!run.properties) run.properties = {};
        run.properties.hidden = true;
        run.properties.suppressRender = true;
      }
    }
  ...
  function patchPPTXSlideRenderer() {
    var Ctor = global.PPTXSlideRenderer;
    if (!Ctor || !Ctor.prototype || Ctor.prototype.__hideFieldsPatched) return false;
    ...
    if (typeof proto.processTextBody === "function") {
      ...
      proto.processTextBody = function (txBodyEl) {
        var textBody = originalProcessTextBody.call(this, txBodyEl);
        return stripFieldRunsFromTextBody(textBody);
      };
    }

    Ctor.prototype.__hideFieldsPatched = true;
    return true;
  }
  ...
  function patchCDrawingDocument() {
    ...
    if (typeof proto.calculateWrappedLines === "function") {
      var originalCalculateWrappedLines = proto.calculateWrappedLines;
      proto.calculateWrappedLines = function (paragraph, paraProps, width, shape) {
        if (paragraph && Array.isArray(paragraph.runs)) {
          var filteredParagraph = {
            runs: paragraph.runs.filter(function (run) {
              return !(run && (run.hidden || run.suppressRender || run.fieldType));
            })
          };

          // preserve any other paragraph properties if present
          for (var k in paragraph) {
            if (k !== "runs" && Object.prototype.hasOwnProperty.call(paragraph, k)) {
              filteredParagraph[k] = paragraph[k];
            }
          }

          return originalCalculateWrappedLines.call(this, filteredParagraph, paraProps, width, shape);
        }

        return originalCalculateWrappedLines.call(this, paragraph, paraProps, width, shape);
      };
    }
  ...
  function install() {
    var ok = false;
    ok = patchPPTXSlideRenderer() || ok;
    ok = patchTableProcessor() || ok;
    ok = patchCDrawingDocument() || ok;
    return ok;
  }
  ...
  function tryInstall() {
    if (install()) {
      global.__OOXML_HIDE_FIELDS_PATCH__ = { installed: true };
      return;
    }
...
/* ooxml-selective-theme-text-color-fix.js
   REPLACEMENT patch for text color inheritance.
   Load AFTER the main bundle and BEFORE loading the PPTX.

   IMPORTANT:
   - Remove previous text-color/theme placeholder patches before using this.
   - This patch preserves explicit/local text colors and only falls back to
     layout/master/theme when a paragraph/run has no local color.
*/
(function (global) {
  "use strict";
  ...
  function cloneRgb(c) {
    if (!isRgb(c)) return null;
    return {
      r: clampByte(c.r),
      g: clampByte(c.g),
      b: clampByte(c.b),
      a: typeof c.a !== "undefined" ? clampByte(c.a) : 255
    };
  }
  ...
  function collectMods(desc) {
    var mods = [];
    if (!desc) return mods;

    if (Array.isArray(desc.modifications)) {
      for (var i = 0; i < desc.modifications.length; i++) {
        var m = desc.modifications[i];
        if (m && m.type) {
          mods.push({ type: String(m.type), value: Number(m.value) });
        }
      }
    }
    ...
    for (var j = 0; j < keys.length; j++) {
      var k = keys[j];
      if (typeof desc[k] !== "undefined") {
        mods.push({ type: k, value: Number(desc[k]) });
      }
    }
  ...
  function applyMods(base, mods) {
    if (!base) return null;
    var out = cloneRgb(base);
    if (!mods || !mods.length) return out;

    var hsl = rgbToHsl(out.r, out.g, out.b);
    var h = hsl.h;
    var s = hsl.s;
    var l = hsl.l;
    var a = typeof out.a !== "undefined" ? out.a : 255;

    for (var i = 0; i < mods.length; i++) {
      var mod = mods[i];
      if (!mod || !isFinite(mod.value)) continue;

      var type = String(mod.type || "").toLowerCase();
      var v = mod.value;
      ...
      switch (type) {
        ...
        case "lummod":
          ...
          break;
        case "lumoff":
          l = Math.max(0, Math.min(1, l + (v / 100000)));
          break;
        case "satmod":
          s = Math.max(0, Math.min(1, s * (v / 100000)));
          break;
        case "satoff":
          s = Math.max(0, Math.min(1, s + (v / 100000)));
          break;
        ...
        case "hueoff":
          ...
          break;
        case "alpha":
          a = v <= 1 ? clampByte(v * 255) : clampByte((v / 100000) * 255);
          break;
        case "alphamod":
          a = clampByte(a * (v / 100000));
          break;
        case "alphaoff":
          a = clampByte(a + 255 * (v / 100000));
          break;
      }
    }
  ...
  function unwrapRenderer(proc) {
    if (!proc) return null;
    if (proc.slides && proc.presentation) return proc;
    if (proc.processor && proc.processor.slides) return proc.processor;
    return proc;
  }

  function getRendererFromDoc(doc) {
    if (doc && doc.processor) return unwrapRenderer(doc.processor);
    if (global.currentProcessor) {
      if (global.currentProcessor.processor) return unwrapRenderer(global.currentProcessor.processor);
      return unwrapRenderer(global.currentProcessor);
    }
    return null;
  }

  function getPresentation(renderer) {
    return renderer && renderer.presentation ? renderer.presentation : null;
  }

  function getTheme(renderer) {
    var p = getPresentation(renderer);
    return p && p.theme ? p.theme : null;
  }

  function getActiveSlide(doc, renderer) {
    if (doc && doc.currentSlide) return doc.currentSlide;
    if (renderer && renderer.currentSlide) return renderer.currentSlide;
    if (renderer && renderer.slides && typeof renderer.currentSlideIndex === "number") {
      return renderer.slides[renderer.currentSlideIndex] || null;
    }
    return null;
  }

  function getColorMap(slide) {
    if (!slide) return null;
    if (slide.layout && slide.layout.clrMapOvr) return slide.layout.clrMapOvr;
    if (slide.layout && slide.layout.master && slide.layout.master.clrMap) return slide.layout.master.clrMap;
    if (slide.master && slide.master.clrMap) return slide.master.clrMap;
    return null;
  }

  function mapSchemeColor(name, colorMap) {
    if (!name) return name;
    if (colorMap && colorMap[name]) return colorMap[name];

    if (name === "bg1") return colorMap && colorMap.bg1 ? colorMap.bg1 : "lt1";
    if (name === "tx1") return colorMap && colorMap.tx1 ? colorMap.tx1 : "dk1";
    if (name === "bg2") return colorMap && colorMap.bg2 ? colorMap.bg2 : "lt2";
    if (name === "tx2") return colorMap && colorMap.tx2 ? colorMap.tx2 : "dk2";

    return name;
  }

  function getThemeColorHex(renderer, slide, scheme) {
    var theme = getTheme(renderer);
    if (!theme || !theme.colors || !scheme) return null;

    var mapped = mapSchemeColor(scheme, getColorMap(slide));
    return theme.colors[mapped] || theme.colors[scheme] || null;
  }

  function getPlaceholder(shape) {
    if (!shape) return null;
    if (shape.placeholder) return shape.placeholder;
    if (shape.nvSpPr && shape.nvSpPr.nvPr && shape.nvSpPr.nvPr.placeholder) {
      return shape.nvSpPr.nvPr.placeholder;
    }
    return null;
  }
  ...
  function getShapeStyle(shape) {
    if (!shape) return null;
    return shape.preservedStyle || shape.style || null;
  }

  function getDefaultTextScheme(shape) {
    var ph = getPlaceholder(shape);
    var type = ph && ph.type ? ph.type : null;
    if (type === "subTitle") return "tx2";
    return "tx1";
  }

  function resolveColorDescriptor(renderer, slide, shape, desc, fallbackScheme) {
    if (!desc) {
      var fallbackHex = getThemeColorHex(renderer, slide, fallbackScheme || getDefaultTextScheme(shape));
      return fallbackHex ? hexToRgb(fallbackHex) : { r: 0, g: 0, b: 0, a: 255 };
    }

    if (isRgb(desc)) return cloneRgb(desc);

    if (typeof desc === "string") {
      if (desc.charAt(0) === "#") return hexToRgb(desc);
      var directHex = getThemeColorHex(renderer, slide, desc);
      return directHex ? hexToRgb(directHex) : null;
    }

    if (desc.type === "srgb" && desc.value) {
      return applyMods(hexToRgb("#" + desc.value), collectMods(desc));
    }

    var scheme =
      desc.scheme ||
      (desc.type === "scheme" ? desc.value : null) ||
      (desc.type === "placeholder" ? "phClr" : null) ||
      (desc.type === "phClr" ? "phClr" : null) ||
      (desc.value === "phClr" ? "phClr" : null);

    if (scheme === "phClr") {
      scheme = fallbackScheme || getDefaultTextScheme(shape);
    }

    if (scheme) {
      var themeHex = getThemeColorHex(renderer, slide, scheme);
      if (themeHex) return applyMods(hexToRgb(themeHex), collectMods(desc));
    }

    if (desc.value && typeof desc.value === "string" && desc.value.charAt(0) === "#") {
      return applyMods(hexToRgb(desc.value), collectMods(desc));
    }
  ...
  function placeholderTypeEq(a, b) {
    if (a === b) return true;
    var compat = {
      title: { ctrTitle: true },
      ctrTitle: { title: true },
  ...
  function placeholdersMatch(a, b) {
    ...
    var ta = a.type || "obj";
    var tb = b.type || "obj";
    if (!placeholderTypeEq(ta, tb)) return false;

    var ia = typeof a.idx === "number" ? a.idx : null;
    var ib = typeof b.idx === "number" ? b.idx : null;

    if (ia != null && ib != null) return ia === ib;
    return true;
  }

  function walkShapeTree(tree, fn) {
    if (!tree || !tree.length) return;
    for (var i = 0; i < tree.length; i++) {
      var s = tree[i];
      fn(s);
      if (s && s.shapeTree && s.shapeTree.length) {
        walkShapeTree(s.shapeTree, fn);
      }
    }
  }

  function findMatchingTemplateShape(shape, tree) {
    if (!shape || !tree || !tree.length) return null;

    var found = null;
    var ph = getPlaceholder(shape);

    if (ph) {
      walkShapeTree(tree, function (candidate) {
        if (found) return;
        var cph = getPlaceholder(candidate);
        if (cph && placeholdersMatch(ph, cph)) found = candidate;
      });
    ...
    if (shape.name) {
      walkShapeTree(tree, function (candidate) {
        if (found) return;
        if (candidate && candidate.name === shape.name) found = candidate;
      });
    }
  ...
  function getTextStyleKind(shape) {
    var ph = getPlaceholder(shape);
    var type = ph && ph.type ? ph.type : null;

    if (type === "title" || type === "ctrTitle") return "titleStyle";
    if (type === "body" || type === "obj") return "bodyStyle";
    return "otherStyle";
  }

  function getFontRefColorDesc(shape) {
    var style = getShapeStyle(shape);
    return style && style.fontRef && style.fontRef.color ? style.fontRef.color : null;
  }

  function getFontRefFamily(renderer, shape) {
    var style = getShapeStyle(shape);
    var fontRef = style && style.fontRef ? style.fontRef : null;
    var theme = getTheme(renderer);
    if (!theme || !theme.fonts) return null;

    var ph = getPlaceholder(shape);
    var idx = fontRef && (fontRef.idx || fontRef.type || fontRef.kind);

    if (idx === "major") return theme.fonts.major || null;
    if (idx === "minor") return theme.fonts.minor || null;

    if (ph && (ph.type === "title" || ph.type === "ctrTitle")) {
      return theme.fonts.major || theme.fonts.minor || null;
    }

    return theme.fonts.minor || theme.fonts.major || null;
  }

  function getListStyleLevel(textBody, level) {
    if (!textBody || !textBody.lstStyle) return null;
    var key = "lvl" + (Number(level || 0) + 1) + "pPr";
    return textBody.lstStyle[key] || textBody.lstStyle.lvl1pPr || null;
  }

  function getLocalListStyleColorDesc(textBody, level) {
    var lvl = getListStyleLevel(textBody, level);
    if (!lvl) return null;
    if (lvl.defRPr && lvl.defRPr.color) return lvl.defRPr.color;
    if (lvl.color) return lvl.color;
    return null;
  }

  function getTemplateDefaultColorDesc(shape, level) {
    if (!shape || !shape.textBody) return null;

    var lvl = getListStyleLevel(shape.textBody, level);
    if (lvl) {
      if (lvl.defRPr && lvl.defRPr.color) return lvl.defRPr.color;
      if (lvl.color) return lvl.color;
    }

    if (shape.textBody.paragraphs && shape.textBody.paragraphs.length) {
      var p = shape.textBody.paragraphs[0];
      if (p && p.properties && p.properties.color) return p.properties.color;
      if (p && p.runs && p.runs.length) {
        for (var i = 0; i < p.runs.length; i++) {
          var run = p.runs[i];
          if (run && run.properties && run.properties.color) return run.properties.color;
        }
      }
    }

    var st = getShapeStyle(shape);
    if (st && st.fontRef && st.fontRef.color) return st.fontRef.color;

    return null;
  }

  function getMasterTxStyleColorDesc(slide, shape, level) {
    if (!slide || !slide.layout || !slide.layout.master || !slide.layout.master.txStyles) return null;

    var txStyles = slide.layout.master.txStyles;
    var kind = getTextStyleKind(shape);
    var wrapper = txStyles[kind];
    if (!wrapper) return null;

    var el = wrapper.element || wrapper;
    if (!el || typeof el.querySelector !== "function") return null;

    var lvlKey = "lvl" + (Number(level || 0) + 1) + "pPr";
    var lvlEl =
      el.querySelector(lvlKey + ", a\\:" + lvlKey) ||
      el.querySelector("lvl1pPr, a\\:lvl1pPr");

    if (lvlEl) {
      var defRPr = lvlEl.querySelector("defRPr, a\\:defRPr");
      if (defRPr) {
        var c1 = defRPr.querySelector("solidFill, a\\:solidFill, schemeClr, a\\:schemeClr, srgbClr, a\\:srgbClr");
        if (c1) {
          if (c1.localName === "solidFill") {
            var cc = c1.querySelector("schemeClr, a\\:schemeClr, srgbClr, a\\:srgbClr");
            if (cc) {
              if (cc.localName === "schemeClr") {
                return { type: "scheme", value: cc.getAttribute("val") };
              }
              if (cc.localName === "srgbClr") {
                return { type: "srgb", value: cc.getAttribute("val") };
              }
            }
          } else if (c1.localName === "schemeClr") {
            return { type: "scheme", value: c1.getAttribute("val") };
          } else if (c1.localName === "srgbClr") {
            return { type: "srgb", value: c1.getAttribute("val") };
          }
        }
      }
    }
  ...
  function computeParagraphDefaultColor(renderer, slide, shape, para, level) {
    // explicit paragraph color wins
    if (para && para.properties && para.properties.color) {
      return resolveColorDescriptor(renderer, slide, shape, para.properties.color, null);
    }

    // local textbox/list style default
    var localListDesc = getLocalListStyleColorDesc(shape.textBody, level);
    if (localListDesc) {
      return resolveColorDescriptor(renderer, slide, shape, localListDesc, null);
    }

    // matching layout/master placeholder defaults
    var layoutTree =
      slide && slide.layout && slide.layout.commonSlideData && slide.layout.commonSlideData.shapeTree
        ? slide.layout.commonSlideData.shapeTree
        : null;

    var masterTree =
      slide && slide.layout && slide.layout.master && slide.layout.master.commonSlideData && slide.layout.master.commonSlideData.shapeTree
        ? slide.layout.master.commonSlideData.shapeTree
        : null;

    var layoutMatch = findMatchingTemplateShape(shape, layoutTree);
    var masterMatch = findMatchingTemplateShape(shape, masterTree);

    var layoutDesc = getTemplateDefaultColorDesc(layoutMatch, level);
    if (layoutDesc) {
      return resolveColorDescriptor(renderer, slide, shape, layoutDesc, null);
    }

    var masterDesc = getTemplateDefaultColorDesc(masterMatch, level);
    if (masterDesc) {
      return resolveColorDescriptor(renderer, slide, shape, masterDesc, null);
    }

    // master txStyles
    var txDesc = getMasterTxStyleColorDesc(slide, shape, level);
    if (txDesc) {
      return resolveColorDescriptor(renderer, slide, shape, txDesc, null);
    }

    // fontRef color
    var fontRefDesc = getFontRefColorDesc(shape);
    if (fontRefDesc) {
      return resolveColorDescriptor(renderer, slide, shape, fontRefDesc, null);
    }

    // theme fallback
    return resolveColorDescriptor(renderer, slide, shape, null, getDefaultTextScheme(shape));
  }

  function computeRunColor(renderer, slide, shape, para, run, level, paraDefaultColor) {
    if (run && run.properties && run.properties.color) {
      return resolveColorDescriptor(renderer, slide, shape, run.properties.color, null);
    }
    if (para && para.properties && para.properties.color) {
      return resolveColorDescriptor(renderer, slide, shape, para.properties.color, null);
    }
    return paraDefaultColor || resolveColorDescriptor(renderer, slide, shape, null, getDefaultTextScheme(shape));
  }

  function materializeShapeTextStyles(renderer, slide, shape) {
    if (!renderer || !slide || !shape || !shape.textBody || !Array.isArray(shape.textBody.paragraphs)) return;

    if (!shape.textBody.bodyProperties) shape.textBody.bodyProperties = {};
    if (!shape.textBody.lstStyle) shape.textBody.lstStyle = {};

    var themeFont = getFontRefFamily(renderer, shape);

    for (var p = 0; p < shape.textBody.paragraphs.length; p++) {
      var para = shape.textBody.paragraphs[p];
      if (!para) continue;
      if (!para.properties) para.properties = {};
      if (!Array.isArray(para.runs)) para.runs = [];

      var level = typeof para.properties.level === "number" ? para.properties.level : 0;
      var paraDefaultColor = computeParagraphDefaultColor(renderer, slide, shape, para, level);

      // store inherited default without pretending it was explicit
      para.properties.__resolvedDefaultColor = paraDefaultColor ? cloneRgb(paraDefaultColor) : null;

      // only materialize paragraph color if paragraph explicitly had one
      if (para.properties.color) {
        para.properties.color = resolveColorDescriptor(renderer, slide, shape, para.properties.color, null);
      }

      if (!para.properties.fontFamily && themeFont) {
        para.properties.fontFamily = themeFont;
      }
      ...
      for (var r = 0; r < para.runs.length; r++) {
        ...
        if (run.properties.color) {
          run.properties.color = resolveColorDescriptor(renderer, slide, shape, run.properties.color, null);
          run.properties.__hasExplicitColor = true;
        } else {
          run.properties.__hasExplicitColor = false;
          run.properties.__resolvedThemeColor = computeRunColor(renderer, slide, shape, para, run, level, paraDefaultColor);
        }

        if (!run.properties.fontFamily && para.properties.fontFamily) {
          run.properties.fontFamily = para.properties.fontFamily;
        } else if (!run.properties.fontFamily && themeFont) {
          run.properties.fontFamily = themeFont;
        }
      }
    }
  }

  function materializeSlideTextStyles(renderer, slide) {
    if (!renderer || !slide || !slide.commonSlideData || !slide.commonSlideData.shapeTree) return;
    walkShapeTree(slide.commonSlideData.shapeTree, function (shape) {
      materializeShapeTextStyles(renderer, slide, shape);
    });
  }

  function patchRenderer() {
    var Ctor = global.PPTXSlideRenderer;
    if (!Ctor || !Ctor.prototype || Ctor.prototype.__selectiveThemeTextColorFixInstalled) return false;

    var proto = Ctor.prototype;
    var originalLinkSlideToLayoutAndMaster = proto.linkSlideToLayoutAndMaster;
    var originalApplyPropertyInheritance = proto.applyPropertyInheritance;

    if (typeof originalLinkSlideToLayoutAndMaster === "function") {
      proto.linkSlideToLayoutAndMaster = async function (slide, part, ctx, slideIndex) {
        var result = await originalLinkSlideToLayoutAndMaster.call(this, slide, part, ctx, slideIndex);
        try {
          materializeSlideTextStyles(this, slide);
        } catch (_) {}
        return result;
      };
    }

    if (typeof originalApplyPropertyInheritance === "function") {
      proto.applyPropertyInheritance = function (shape, slide) {
        originalApplyPropertyInheritance.call(this, shape, slide);
        try {
          var activeSlide = slide || this.currentSlide || null;
          if (activeSlide) materializeShapeTextStyles(this, activeSlide, shape);
        } catch (_) {}
      };
    }

    Ctor.prototype.__selectiveThemeTextColorFixInstalled = true;
    return true;
  }

  function patchDrawingDocument() {
    var Ctor = global.CDrawingDocument;
    if (!Ctor || !Ctor.prototype || Ctor.prototype.__selectiveThemeTextColorFixInstalled) return false;

    var proto = Ctor.prototype;
    var originalParseRunProperties = proto.parseRunProperties;
    var originalResolveInheritedTextProperties = proto.resolveInheritedTextProperties;
    var originalSetupStandardFont = proto.setupStandardFont;
    var originalGetDefaultTextColor = proto.getDefaultTextColor;

    if (typeof originalParseRunProperties === "function") {
      proto.parseRunProperties = function (run, paraProps, shape) {
        var props = originalParseRunProperties.call(this, run, paraProps, shape) || {};

        try {
          var currentShape = shape || (typeof this.getCurrentShape === "function" ? this.getCurrentShape() : null);

          // Preserve explicit run color if present.
          if (props.color) {
            return props;
          }

          if (run && run.properties) {
            if (run.properties.__hasExplicitColor && run.properties.color) {
              props.color = cloneRgb(run.properties.color);
              return props;
            }

            if (!props.color && run.properties.__resolvedThemeColor) {
              props.color = cloneRgb(run.properties.__resolvedThemeColor);
            }
          }

          if (!props.color && paraProps && paraProps.color) {
            props.color = cloneRgb(paraProps.color);
          }

          if (!props.color && paraProps && paraProps.__resolvedDefaultColor) {
            props.color = cloneRgb(paraProps.__resolvedDefaultColor);
          }

          if (!props.fontFamily && run && run.properties && run.properties.fontFamily) {
            props.fontFamily = run.properties.fontFamily;
          } else if (!props.fontFamily && paraProps && paraProps.fontFamily) {
            props.fontFamily = paraProps.fontFamily;
          }

          if (!props.color && typeof originalGetDefaultTextColor === "function" && currentShape) {
            props.color = originalGetDefaultTextColor.call(this, currentShape);
          }
        } catch (_) {}
    ...
    if (typeof originalResolveInheritedTextProperties === "function") {
      proto.resolveInheritedTextProperties = function (shape, kind, level) {
        var out = originalResolveInheritedTextProperties.call(this, shape, kind, level) || {};
        try {
          if (!out.color) {
            var renderer = getRendererFromDoc(this);
            var slide = getActiveSlide(this, renderer);
            if (renderer && slide && shape) {
              out.color = computeParagraphDefaultColor(renderer, slide, shape, { properties: { level: level || 0 } }, level || 0);
            }
          }
        } catch (_) {}
        return out;
    ...
    if (typeof originalSetupStandardFont === "function") {
      proto.setupStandardFont = function (runProps) {
        if (runProps && !runProps.color && runProps.__resolvedThemeColor) {
          runProps.color = cloneRgb(runProps.__resolvedThemeColor);
        }
        return originalSetupStandardFont.call(this, runProps);
      };
    }

    if (typeof originalGetDefaultTextColor === "function") {
      proto.getDefaultTextColor = function (shape) {
        try {
          var renderer = getRendererFromDoc(this);
          var slide = getActiveSlide(this, renderer);
          if (renderer && slide && shape) {
            var c = computeParagraphDefaultColor(renderer, slide, shape, { properties: {} }, 0);
            if (c) return c;
          }
        } catch (_) {}
        return originalGetDefaultTextColor.call(this, shape);
      };
    }
  ...
  function tryInstall() {
    if (install()) {
      global.__OOXML_SELECTIVE_THEME_TEXT_COLOR_FIX__ = { installed: true };
      return;
    }
...
/* ooxml-merged-cell-text-fix.js
   Load AFTER your pptx viewer bundle and BEFORE rendering slides.
   Fix: prevent repeated text in merged table cells.
*/
(function (global) {
  "use strict";

  function hasVisibleText(textBody) {
    if (!textBody || !Array.isArray(textBody.paragraphs)) return false;
    ...
    for (var p = 0; p < textBody.paragraphs.length; p++) {
      ...
      for (var r = 0; r < para.runs.length; r++) {
        ...
        if (txt.replace(/\u00A0/g, " ").trim().length > 0) return true;
      }
    }
  ...
  function cloneParagraphs(textBody) {
    if (!textBody || !Array.isArray(textBody.paragraphs)) return [];
    return textBody.paragraphs.map(function (p) {
      return {
        properties: p && p.properties ? Object.assign({}, p.properties) : {},
        runs: Array.isArray(p && p.runs)
          ? p.runs.map(function (r) {
              return {
                text: r ? r.text : "",
                fieldType: r ? r.fieldType : undefined,
                properties: r && r.properties ? Object.assign({}, r.properties) : {}
              };
            })
          : []
      };
  ...
  function installPatch() {
    var Ctor = global.TableProcessor;
    if (!Ctor || !Ctor.prototype) return false;
    if (Ctor.prototype.__mergedCellTextFixInstalled) return true;

    var proto = Ctor.prototype;

    proto.appendMergedCellText = function (parentCell, childCell) {
      if (!childCell || childCell.mergedContentTransferred) return;

      var childTextBody = childCell.getTextBody && childCell.getTextBody();
      if (!childTextBody || !childTextBody.paragraphs || childTextBody.paragraphs.length === 0) {
        return;
      }

      if (!parentCell || !parentCell.getTextBody || !parentCell.setTextBody) return;

      var parentTextBody = parentCell.getTextBody && parentCell.getTextBody();
      var parentAlreadyHasText = hasVisibleText(parentTextBody);

      // Only transfer child text if parent is effectively empty.
      // This prevents duplicated text in merged cells.
      if (!parentAlreadyHasText) {
        var copied = cloneParagraphs(childTextBody);

        if (!parentTextBody || !Array.isArray(parentTextBody.paragraphs)) {
          parentCell.setTextBody({ paragraphs: copied });
        } else {
          parentTextBody.paragraphs = copied;
        }
      }

      // Always suppress continuation-cell text after merge handling
      childCell.mergedContentTransferred = true;
      if (childCell.setTextBody) {
        childCell.setTextBody({ paragraphs: [] });
      }
    };

    Ctor.prototype.__mergedCellTextFixInstalled = true;
    return true;
  }
  ...
  function tryInstall() {
    if (installPatch()) {
      global.__OOXML_MERGED_CELL_TEXT_FIX__ = { installed: true };
      return;
    }
...
/* ooxml-table-text-style-fix.js
   Load AFTER your viewer bundle and BEFORE rendering.
*/
(function (global) {
  "use strict";
  ...
  function fillMissing(target, source) {
    ...
    if (target.fontSize == null && source.fontSize != null) target.fontSize = source.fontSize;
    if (!target.fontFamily && source.fontFamily) target.fontFamily = source.fontFamily;
    if (typeof target.bold === "undefined" && typeof source.bold !== "undefined") target.bold = source.bold;
    if (typeof target.italic === "undefined" && typeof source.italic !== "undefined") target.italic = source.italic;
    if (typeof target.underline === "undefined" && typeof source.underline !== "undefined") target.underline = source.underline;
    if (typeof target.strikethrough === "undefined" && typeof source.strikethrough !== "undefined") target.strikethrough = source.strikethrough;
    if (!target.color && source.color) target.color = source.color;

    return target;
  }

  function getParagraphElements(txBody) {
    var out = [];
    if (!txBody || !txBody.children) return out;

    for (var i = 0; i < txBody.children.length; i++) {
      var el = txBody.children[i];
      var name = (el.localName || el.tagName || "").toLowerCase();
      if (name === "p") out.push(el);
    }
    return out;
  }

  function parseDefaultRunProps(proc, pEl) {
    if (!pEl || typeof pEl.querySelector !== "function") return null;

    var pPr = pEl.querySelector("pPr, a\\:pPr");
    var rPr =
      (pPr && pPr.querySelector("defRPr, a\\:defRPr")) ||
      pEl.querySelector("endParaRPr, a\\:endParaRPr");
    ...
    if (sz) {
      ...
      if (!isNaN(n)) out.fontSize = n / 100;
    }

    var latin = rPr.querySelector("latin, a\\:latin");
    var family =
      rPr.getAttribute("typeface") ||
      rPr.getAttribute("fontFamily") ||
      (latin && latin.getAttribute("typeface"));

    if (family) out.fontFamily = family;

    if (rPr.hasAttribute("b")) {
      var b = rPr.getAttribute("b");
      out.bold = b !== "0" && b !== "false";
    }

    if (rPr.hasAttribute("i")) {
      var it = rPr.getAttribute("i");
      out.italic = it !== "0" && it !== "false";
    }

    if (rPr.hasAttribute("u")) {
      out.underline = rPr.getAttribute("u") !== "none";
    }

    if (rPr.hasAttribute("strike")) {
      out.strikethrough = rPr.getAttribute("strike") !== "noStrike";
    }

    if (typeof proc.parseColor === "function") {
      var color = proc.parseColor(rPr);
  ...
  function installPatch() {
    var Ctor = global.TableProcessor;
    if (!Ctor || !Ctor.prototype) return false;
    if (Ctor.prototype.__tableTextStyleFixInstalled) return true;

    var proto = Ctor.prototype;
    if (typeof proto.parseTextBodyFromElement !== "function") return false;

    var original = proto.parseTextBodyFromElement;

    proto.parseTextBodyFromElement = function (el) {
      var textBody = original.call(this, el);
      if (!textBody || !Array.isArray(textBody.paragraphs)) return textBody;

      var txBody = el && el.querySelector
        ? el.querySelector("txBody, a\\:txBody")
        : null;
      if (!txBody) return textBody;

      var paragraphEls = getParagraphElements(txBody);

      for (var i = 0; i < textBody.paragraphs.length && i < paragraphEls.length; i++) {
        var para = textBody.paragraphs[i];
        var defaults = parseDefaultRunProps(this, paragraphEls[i]);
        if (!defaults) continue;

        para.properties = fillMissing(para.properties || {}, defaults);

        if (!Array.isArray(para.runs)) continue;
        for (var j = 0; j < para.runs.length; j++) {
          var run = para.runs[j];
          if (!run) continue;
          run.properties = fillMissing(run.properties || {}, defaults);
        }
      }
    ...
    Ctor.prototype.__tableTextStyleFixInstalled = true;
    return true;
  }
  ...
  (function tryInstall() {
    if (installPatch()) {
      global.__OOXML_TABLE_TEXT_STYLE_FIX__ = { installed: true };
      return;
    }
    attempts += 1;
    if (attempts < maxAttempts) setTimeout(tryInstall, 50);
  })();
})(typeof globalThis !== "undefined" ? globalThis : window);